...
const RadioButton = ({ submission, setSubmission }) => {
  ...
  const setOption = newOption => {
    const newOptions = submission.options.map (option =>
      option.id === newOption.id ? newOption :
        newOption.correct ? { ...option, correct : false } : option
    )
    setSubmission ({ ...submission, options : newOptions })
  }
